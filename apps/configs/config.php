...
$GLOBALS = Array();
$GLOBALS['root_directory'] = getenv("DOCUMENT_ROOT") . DIRECTORY_SEPARATOR . 'ritesh' . DIRECTORY_SEPARATOR;
...
if ($_SESSION['user_informations']['uid'] != '') {
	...
	//echo $_SESSION['user_informations']['uid'];
	$user = new User();
	//print_r($_SESSION['user_informations']['uid']);
	//
	$user_info = $user->fetchRow("id = " . $_SESSION['user_informations']['uid']);
	
	//print_r($user_info->toArray());
	$GLOBALS['Smarty']->assign('user_info', $user_info->toArray());
...
$dotposition = strpos($_SERVER['REQUEST_URI'], ".");
$template_url = substr($_SERVER['REQUEST_URI'], 13, $dotposition - 13) . ".tpl";
$GLOBALS['Smarty']->assign('uri', substr($_SERVER['REQUEST_URI'], 15, $dotposition - 15));